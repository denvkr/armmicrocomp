  ...
  <div class="widget-table-wrapper">
    ...
    <div class="widget-table-body">
      <div class="widget-table-action">
        ...
        <a href="javascript:void(0);" class="get-csv tool-tip-top" title="<?php echo $lang['T_TIP_PRODUCT_CSV'];?>"><span><?php echo $lang['IN_CSV'];?></span></a>
      ...
      <div class="import-container">
        ...
        <div class="block-upload-сsv">
          ...
          <select name='importType'>
            <option value="MogutaCMS">Moguta.CMS</option>      
          </select>
          <a href="<?php echo SITE?>/mg-admin?examplecsv=1" class="get-example-csv"><?php echo $lang['EXAMPLE_CATALOG_CSV'];?></a>
        ...
        <div class="block-importer">
          ...
          <div class="repeat-upload-file"><a href="javascript:void(0);" class="repeat-upload-csv"></a></div>
          ...
          <div class="delete-all-products-btn"><input type="checkbox" name="no-merge" class="" title="<?php echo $lang['CLEAR_BEFORE_CSV'];?>" checked="checked" value="true"><?php echo $lang['DEL_ALL_PROD'];?></div>
      ...
      <div class="main-settings-container">
        <table class="widget-table product-table">
          ...
          foreach($catalog as $data){ ?>
            <tr id="<?php echo $data['id'] ?>" data-id="<?php echo $data['id'] ?>">
                ...
                <td class="price">
                   <?php if(!empty($data['variants'])){
                    foreach ($data['variants'] as $item) {
                      echo '<span class="price-help">'.$item['title_variant'].'</span> <input style="width: 45px;" class="variant-price fastsave" type="text" value="'.$item['price'].'" data-packet="{variant:1,id:'.$item['id'].',field:\'price\'}"/> '.$settings['currency'].'<div class="clear"></div>';
                    }             
                  }else{
                    echo '<input style="width: 45px;" type="text" value="'.$data['price'].'" class="fastsave" data-packet="{variant:0,id:'.$data['id'].',field:\'price\'}"/> '.$settings['currency'];                               
                  }?>
                </td>
                <td class="count">
                   <?php if(!empty($data['variants'])){
                    foreach ($data['variants'] as $item) {
                      echo '<input style="width: 25px;" class="variant-count fastsave" type="text" value="'.($item['count']<0?'&#8734;':$item['count']).'" data-packet="{variant:1,id:'.$item['id'].',field:\'count\'}"/> '.$lang['UNIT'].'<div class="clear"></div>';
                    }             
                  }else{
                    echo '<input style="width: 25px;" type="text" value="'.($data['count']<0?'&#8734;':$data['count']).'" class="fastsave" data-packet="{variant:0,id:'.$data['id'].',field:\'count\'}"/> '.$lang['UNIT'];                                
                  }?>                                    
                </td>                                    
                <td class="actions">
      ...
      <a href="javascript:void(0);" class="removeAllProduct"><span><?php echo $lang['DELL_SELECTED_PROD']?></span></a>
      <?php echo $pagination ?>
      <div class="clear"></div>
    </div>
            ...
            <div class="add-product-form-wrapper">
              ...
              <div class="add-img-form">
                ...
                <div class="product-text-inputs">
                  <label for="title"><span class="custom-text"><?php echo $lang['NAME_PRODUCT'];?>:</span><input style="width:248px;" type="text" name="title" class="product-name-input tool-tip-right" title="<?php echo $lang['T_TIP_NAME_PROD'];?>" ><div class="errorField"><?php echo $lang['ERROR_SPEC_SYMBOL'];?></div></label>
                  ...
                  <div class="category-filter">
                    <span class="custom-text"><?php echo $lang['CAT_PRODUCT'];?>:</span>
