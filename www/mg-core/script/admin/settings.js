  ...
  return {
    ...
    */
    init: function() {
      ...
      $('.admin-center').on('click', '.section-settings .file-template #tab-template-settings>.file-template', function(){
    ...
    */
    ...
    openPaymentModalWindow: function(id) {
     var paramArray = JSON.parse($('tr[id=payment_'+id+'] td#paramHideArray').html());
      ...
      $('#add-paymentMethod-wrapper span#paymentName').html('<span class="'+iconClass+'">'+$('tr[id=payment_'+id+'] td#paymentName').text()+'</span>');
      ...
      $.each(paramArray, function(name, val) {  
         $('#add-paymentMethod-wrapper #paymentParam').append(
          '<label>\n\
            ...
            <input type="text" name="'+name+'" class="product-name-input" value="">\n\
          </label>'
         );
